    ...
    <div v-for="postItem in postData" class="flex-col h-auto w-full pb-8">
      <t-tooltip :content="showTitle + postItem.title">
        ...
        >
          ...
          <template v-if="commentNumberOfPost.find(comment => comment.id === postItem.id)?.number > 0"
                    #reply>
...
const handleGetPostData = async () => {
  ...
  const {data} = await useFetch(`${import.meta.env.VITE_API_ENDPOINT}` + '/auth/posts/1', {
    method: 'GET',
    headers: {
...
const handleGetCommentData = async () => {
  ...
  const {data} = await useFetch(`${import.meta.env.VITE_API_ENDPOINT}` + '/auth/comments/1', {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${userInfo.token}`
