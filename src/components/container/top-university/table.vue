<template>
  ...
  <div v-else class="flex-col h-auto w-full pb-8">
    ...
    <div class="flex flex-wrap justify-between w-full h-full">
      <VChart v-if="!isLoading" class="mt-8 w-[calc(50%-0.5em)] h-1/2" :option="academicOption" :key="chartKey" autoresize/>
      <VChart v-if="!isLoading" class="mt-8 w-[calc(50%-0.5em)] h-1/2" :option="employerOption" :key="chartKey" autoresize/>
    </div>
  </div>
</template>
...
const academicOption = ref({
  title: {
    text: "University 2021 ~ 2024 學術分數",
    left: "center"
  },
  tooltip: {
    trigger: "item",
...
const employerOption = ref({
  title: {
    text: "University 2021 ~ 2024 師資分數",
    left: "center"
  },
  tooltip: {
    trigger: "item",
...
// changeOption is changing the score data
const changeOption = async () => {
  for (let i = 0; i < universityResData.value.length; i++) {
    ...
    option.value.series[i] = {
      ...
      type: 'line',
      data: universityResData.value[i].overall_info_score_list,
    }
    ...
    academicOption.value.series[i] = {
      name: universityResData.value[i].university_name,
      type: 'line',
      data: universityResData.value[i].academic_reputation_info_score_list,
    }
    ...
    employerOption.value.series[i] = {
      name: universityResData.value[i].university_name,
      type: 'line',
      data: universityResData.value[i].employer_reputation_info_score_list,
    }
